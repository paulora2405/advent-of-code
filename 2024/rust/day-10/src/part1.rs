    ...
    starts
        ...
        .map(|start| {
            hike(
                trail,
                *start,
                &mut HashSet::with_capacity((trail.n * trail.m) as usize),
            )
        })
        .sum()
}
...
#[tracing::instrument]
fn hike(trail: &Trail, pos: (i32, i32), seen: &mut HashSet<(i32, i32)>) -> u32 {
    seen.insert(pos);
    ...
    Dir::all()
        .iter()
        .map(|d| {
            if trail.is_hikable(pos, d) && !seen.contains(&d.going_from(pos)) {
                hike(trail, d.going_from(pos), seen)
            } else {
                0
            }
        })
